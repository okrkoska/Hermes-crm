    ...
    <div style={{ display: "flex", gap: 10, justifyContent: "center", margin: "12px 0" }}>
      {[0,1,2,3].map(i => (
        <div key={i} style={{
          ...
          background: "#0B1525", display: "flex", alignItems: "center", justifyContent: "center",
          fontSize: 22, color: "#E2E8F0", letterSpacing: 2, transition: "border-color .2s"
        }}>
          {value.length > i ? "●" : ""}
        </div>
      ))}
      <input
        type="number" maxLength={4} value={value}
        ...
        <div style={{ padding:"24px 28px 28px" }}>
          ...
          {step === "pin" && (
            <>
              ...
              <div style={{ fontSize:11, color:"#1E3A5F", textAlign:"center", marginBottom:12 }}>Default: 2026</div>
...
function SettingsPanel({ session, lang, setLang, onSignOut, t, theme, setTheme, th }) {
  ...
  const inp = (label, val, set) => (
    <div style={{ marginBottom:14 }}>
      <label style={{ display:"block", fontSize:11, color:"#4B6280", textTransform:"uppercase", letterSpacing:".06em", marginBottom:5 }}>{label}</label>
      <input type="password" value={val} onChange={e => { set(e.target.value.replace(/\D/g,"").slice(0,4)); setMsg(null); }}
        maxLength={4} placeholder="••••"
        style={{ width:"100%", padding:"9px 12px", borderRadius:7, border:`1px solid ${th.border}`, background:th.inputBg, color:th.text, fontSize:20, letterSpacing:8, boxSizing:"border-box", outline:"none" }} />
    </div>
  );
